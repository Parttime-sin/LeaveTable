...
interface SettingsPageProps {
  ...
  onSaveSettings: (next: MonthlySettings) => void;
...
const SettingsPage: React.FC<SettingsPageProps> = ({
  ...
  const [local, setLocal] = useState<MonthlySettings>(settings);
  ...
  const push = (next: MonthlySettings) => {
    setLocal(next);
    onSaveSettings(next);
  };

  const handleMonthChange = (increment: number) => {
    const curr = new Date(year, month0);
    curr.setMonth(curr.getMonth() + increment);
    onChangeMonth(toMonthKey(curr.getFullYear(), curr.getMonth()));
  };
  ...
  const handleRemoveMember = (name: string) => {
    if (!isEditing) return;
    push({ ...local, members: local.members.filter((m) => m !== name) });
  };

  const handleSetPattern = (day: 1 | 2) => {
    if (!isEditing) return;
    const monthStart = startOfMonth(new Date(year, month0));
    const target = day === 1 ? monthStart : addDays(monthStart, 1);
    // NOTE: clearing dailyQuotas on pattern change — previous quotas for old pattern are no longer valid.
    push({
      ...local,
      firstWorkDay: format(target, 'yyyy-MM-dd'),
      dailyQuotas: {},
    });
  };
  ...
  const handleApplyRange = (type: 'work' | 'rest' | 'clear') => {
    ...
    const nextExceptions = { ...local.shiftExceptions };
    eachDayOfInterval({ start, end }).forEach((day) => {
      const dateStr = format(day, 'yyyy-MM-dd');
      if (type === 'clear') delete nextExceptions[dateStr];
      else nextExceptions[dateStr] = type === 'work';
    });
    push({ ...local, shiftExceptions: nextExceptions });
    alert('排班範圍設定已更新');
  };

  const isWorkDay = (date: Date) =>
    computeIsWorkDay(format(date, 'yyyy-MM-dd'), local.firstWorkDay, local.shiftExceptions);
  ...
  const handleBulkSet = (quota: number) => {
    if (!isEditing || !local.firstWorkDay) return;
    const nextQuotas = { ...local.dailyQuotas };
    daysInMonth.forEach((day) => {
      if (isWorkDay(day)) {
        const dateStr = format(day, 'yyyy-MM-dd');
        if (quota === 0) delete nextQuotas[dateStr];
        else nextQuotas[dateStr] = quota;
      }
    });
    push({ ...local, dailyQuotas: nextQuotas });
  };

  const applyBulkQuota = () => {
    if (!isEditing) return;
    const val = parseFloat(bulkQuotaValue);
    if (!isNaN(val) && val >= 0) handleBulkSet(val);
  };
  ...
  const saveSingleQuota = () => {
    ...
    const nextQuotas = { ...local.dailyQuotas };
    if (isNaN(val) || val === 0) delete nextQuotas[editingDate];
    else nextQuotas[editingDate] = val;
    push({ ...local, dailyQuotas: nextQuotas });
    setEditingDate(null);
    setSingleQuotaValue('');
  };

  const hasSetting = !!local.firstWorkDay;
  const activePattern = !hasSetting ? null : isWorkDay(monthStart) ? 1 : 2;

  const groupColorClass = group === 'A' ? 'text-indigo-600' : 'text-teal-600';
  const groupBgClass = group === 'A' ? 'bg-indigo-50 border-indigo-200' : 'bg-teal-50 border-teal-200';
  const groupButtonClass = group === 'A'
    ? 'bg-indigo-600 hover:bg-indigo-700 focus:ring-indigo-600'
    : 'bg-teal-600 hover:bg-teal-700 focus:ring-teal-600';
  const groupFocusClass = group === 'A'
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        ...
        >
          {isEditing ? (
            <><Save className="w-5 h-5 mr-2" />儲存並鎖定</>
          ) : (
            <><Edit className="w-5 h-5 mr-2" />啟用編輯模式</>
          )}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="bg-white shadow rounded-lg p-6 relative overflow-hidden">
            <div className={`absolute top-0 left-0 w-1 h-full ${group === 'A' ? 'bg-indigo-500' : 'bg-teal-500'}`}></div>
          ...
          <div className="bg-white shadow rounded-lg p-6 relative overflow-hidden">
            ...
            <div className="max-h-[300px] overflow-y-auto space-y-2">
              {local.members.length === 0 && (
                <p className="text-sm text-gray-400 text-center py-4">暫無 {group} 班人員資料</p>
              )}
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-amber-700">
                      請先在左側設定「{group}班當月首日上班日」以啟用配額設定功能。
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className={`bg-slate-50 border border-slate-200 rounded-lg p-4 mb-4 ${opacityClass}`}>
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-end sm:items-center">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    ...
                    <button
                      onClick={() => {
                        if (!window.confirm('確定要清除本月所有配額嗎？')) return;
                        handleBulkSet(0);
                      }}
                      disabled={!isEditing}
            ...
            <div className="flex-1 min-h-[500px]">
              <div className="grid grid-cols-7 gap-px bg-gray-200 border border-gray-200 rounded-lg overflow-hidden h-full">
                ...
                {daysInMonth.map((day, dayIdx) => {
                  ...
                  const quota = local.dailyQuotas[dateStr];
                  ...
                  return (
                    <div
                      ...
                      style={colSpanStyle}
                      onClick={() => {
                        if (isEditing && local.firstWorkDay && workDay) {
